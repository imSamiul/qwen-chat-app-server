...
export async function handleSendFriendRequest(req: Request, res: Response) {
  try {
    ...
    const senderObjectId = new Types.ObjectId(senderId as string);
    const recipientObjectId = new Types.ObjectId(recipientId as string);
    ...
    const sender = await UserModel.findById(senderObjectId);
    socketStore.emitToUser(recipientObjectId.toString(), 'newFriendRequest', {
